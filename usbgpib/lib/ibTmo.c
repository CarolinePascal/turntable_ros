/***************************************************************************
 ...
 adapted from lib/ibTmo.c of the Linux GPIB Package driver 
 							by Frank Mori Hess      
...
{
	setIberr( usec_to_timeout( conf->settings.usec_timeout ) );
...
{
	if(usec_timeout)
		usec_timeout = (usec_timeout>=LINETIMEOUT)? (usec_timeout+ADDTIMEOUT):usec_timeout;
