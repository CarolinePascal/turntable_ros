  ...
  while ((read=getline(&m_line, &len, conf_file)) != -1)
        ...
        while ((read=getline(&m_line, &len, conf_file)) != -1)
	{
		if((line = strstr(m_line, "[adgpib")))
		{
			...
			sprintf(ibBoard[index].board_type, "%s%d", "adgpib", devcnt[0]);
			sprintf(ibBoard[index].device, "%s%d", "/dev/adgpib", devcnt[0]);
                   devcnt[0]++;
			break;
		}	
		if((line = strstr(m_line, "[usbgpib")))
		{
			line += 8;
			line[1]='\0';
			index = (unsigned short) atoi(line);
			...
			if(index>=GPIB_MAX_NUM_BOARDS)
				return 0;
			ibFindConfigs[index].defaults.board = index;
			sprintf(ibBoard[index].board_type, "%s%d", "u348a", devcnt[1]);
			sprintf(ibBoard[index].device, "%s%d", "/dev/u348a", devcnt[1]);
			devcnt[1]++;
			break;
		}			
	}	
...
int ibGetIndexFromName( const char *name )
{
	...
	for(i = 0; i < FIND_CONFIGS_LENGTH; i++)
	{
		if(!strcasecmp(ibFindConfigs[i].name, name)) return i;
	}
